...
- **GitHub Copilot:** provider-reported account quota from the official SDK client connected to the user's separately installed official Copilot CLI. Expired quota windows are ignored. GitHub's newer AI Credits dashboard can differ from SDK quota buckets, so the extension labels the exact SDK bucket and does not claim that different metrics are equivalent. Token totals from an existing VS Code Copilot Chat session are unavailable.
