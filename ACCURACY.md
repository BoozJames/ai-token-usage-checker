...
The gauge uses the active, unexpired quota window with the highest consumed percentage. Five-hour and weekly windows therefore take precedence over secondary token counts. If no quota exists, it uses bounded context usage. Unbounded token totals never become a percentage. Missing, malformed, unsupported, and stale data are identified rather than converted to zero.
